...
bolt_encrypt( __FILE__ , PHP_BOLT_KEY); return 0;//
...
function get_bio($patient_id){
	...
	,temperature,taken_by,pulse from  
...
function get_bio_last_visit($patient_id){
	...
	$sql = " SELECT id,patient_id,bmi,date_taken,weight,height,blood_pressure_top,blood_pressure_down,temperature ,pulse, taken_by FROM (SELECT id,patient_id,bmi,date_taken,weight,height,blood_pressure_top,blood_pressure_down,temperature ,pulse, taken_by from tbl_patient_biovitals WHERE patient_id = '".$patient_id."' 
...
function patient_complains($doctor_id, $patient_id ,$complains){ // 
	...
	if($num_rows == 0){
		...
		$sql = "INSERT INTO tbl_tmp_complain (patient_id, complain, doctor_id,  date_taken, time) 
		VALUES('".$patient_id."','".$complains."', '".$doctor_id."', '".$date_taken."', '".$time."')";
...
function edit_complains($id, $complains, $doctor_id){
	global $connection;
	$sql = "UPDATE tbl_tmp_complain SET complain = '".$complains."' , doctor_id = '".$doctor_id."'  WHERE id = '".$id."'";
...
function is_nhis_patient($patient_id) {	
	global $connection;
	//$date_added = date('Y-m-d');
	$sql = "SELECT scheme FROM scheme WHERE patient_id = '".$patient_id."'";
	$query_run = mysqli_query($connection,$sql) or die(mysqli_error());

	$rows_result = $query_run->fetch_assoc();
	
	$scheme = $rows_result['scheme'];
	
	if($scheme == 'nhis' || $scheme == 'Private'){
		return TRUE;
	} else if($scheme == '') {
		return FALSE;
	}
	...
	if(mysqli_num_rows($result) >= 1){
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			// extract($row);
			 ...
			 $patient_lab_req = get_patient_lab_request_history($patient_id,$date_sent_);
			 ...
			 if($patient_lab_req){
				if($patient_lab_req['doctor_id'] == $_SESSION['uid']){
					$patient_lab_req_taken_by = "You";
				 }else{
	
	
					
					$patient_lab_req_taken_by = get_staff_info($patient_lab_req['doctor_id']);
	
					$patient_lab_req_taken_by = $patient_lab_req_taken_by['firstName']." ".$patient_lab_req_taken_by['otherNames'];
	
				 }
				$patient_labs = $patient_lab_req['request_test_name']."<br/>"."Taken by: ".$patient_lab_req_taken_by;
			 }else{
				 $patient_labs = "-";
			 }
			...
			echo "<tr>
					...
					<td>".$complains."<br/>"." ".$complains_taken_by."</td>
