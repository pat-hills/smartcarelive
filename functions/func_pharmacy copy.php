...
function add_drug($a,$b,$c,$d,$e,$f,$g,$gdrg,$manu){
	...
	$sql="INSERT INTO tbl_drug_list (drug_code,Name,NHIS,Expiry_date,quantity,type,price,gdrg,manufacture,product_code,date_added)
	VALUES ('".$drug_code."','".$b."','".$c."','".$d."','".$e."','".$f."','".$g."','".$gdrg."','".$manu."','".$a."','".date('Y-m-d')."')";
...
<td style='font-size:15px; '><span class='label label-primary'>".getDrugName($see['drug_code'])."</span></td>
		...
		<td><strong class='badge badge-success'>".$see['quantity']." X ".$see['times']." ".$see['rate']."
		 ".$see['time_interval']."(s)&nbsp; For ".$see['duration']."&nbsp;".$see['time_span']."</strong></td>";
...
echo "<input type='submit' name='SendToCashier' value='Send To Cashier' style='float:right;' class='btn btn-primary senddata'>";
return; 
 	 	}
			}
}else{
...
echo "<input type='submit' name='SendToCashier' value='Send To Cashier' style='float:right;' class='btn btn-primary'>";
...
function getAllavailableDrugs(){
	global $connection;
$theSQL = "SELECT Name,quantity,price,Expiry_date,drug_code FROM tbl_drug_list";
