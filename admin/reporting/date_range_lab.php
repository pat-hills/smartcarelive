...
if(!empty($startDate) && empty($endDate)){
  $pageWrite = $clientType." "."Investigations As At ".date("F jS, Y", strtotime($startDate));   
}

if(!empty($startDate) && !empty($endDate)){
   $pageWrite = $clientType." "."Investigations from ".date("F jS, Y", strtotime($startDate)). " To ".date("F jS, Y", strtotime($endDate));
}
...
$get_all_consultation = date_range_investigation($startDate,$endDate,$clientType);
...
$pdf->writeHTML($pageWrite, true, 0, true, 0, 'L');
...
<tbody>
      <tr style="background-color: #666; color: #fff;">
       <td style="text-align: left; width: 20%;"><strong>Patient ID</strong></td>
        
         <td style="text-align: left; width: 20%;"><strong>Patient Name</strong></td>
         <td style="text-align: left; width: 20%;"><strong>Scheme</strong></td>
          <td style="text-align: left; width: 20%;"><strong>Investigations</strong></td>
...
$row_count = 0; // Initialize total tests count
while ($row = mysqli_fetch_array($get_all_consultation)) {
   ...
   // $total_tests += $test_count;
    ...
    <tbody>
        <tr>
          <td style="width: 20%; text-align: left;">' . html_entity_decode(htmlentities($row["patient_id"],ENT_QUOTES),ENT_QUOTES) . '</td>
            <td style="width: 20%; text-align: left;">' . html_entity_decode(htmlentities(patient_name($row["patient_id"]),ENT_QUOTES),ENT_QUOTES) . '</td>
                         <td style="width: 20%; text-align: left;">' . html_entity_decode(htmlentities(!empty($row["scheme"]) ? $row["scheme"] : "CASH",ENT_QUOTES),ENT_QUOTES) . '</td>
            <td style="width: 20%; text-align: left;">' . html_entity_decode(htmlentities($row["request_test_name"],ENT_QUOTES),ENT_QUOTES) . '</td>
           <td style="width: 20%; text-align: left;">' . html_entity_decode(htmlentities(date("F jS, Y", strtotime($row['processed_date'])),ENT_QUOTES),ENT_QUOTES) . '</td>
        </tr>
    </tbody>';
...
<tr>
<td style="text-align: left; width: 80%;"><strong>TOTAL RECORDS</strong></td>  
 <td style="text-align: left; width: 20%;">'. htmlentities("". $row_count) . '</td>
</tr>
</tbody>';
