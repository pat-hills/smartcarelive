...
if(!empty($startDate) && empty($endDate)){
    $pageWrite = $clientType." "."Drugs dispensed As At ".date("F jS, Y", strtotime($startDate));   
}

if(!empty($startDate) && !empty($endDate)){
   $pageWrite = $clientType." "."Drugs dispensed from ".date("F jS, Y", strtotime($startDate)). " To ".date("F jS, Y", strtotime($endDate));
...
$get_all_consultation = date_range_medication($startDate,$endDate,$clientType);
      ...
      <tr style="background-color: #666; color: #fff;">
       <td style="text-align: left; width: 20%;"><strong>Patient ID</strong></td>
         ...
         <td style="text-align: left; width: 20%;"><strong>Scheme</strong></td>
          <td style="text-align: left; width: 20%;"><strong>Drugs</strong></td>
...
while ($row = mysqli_fetch_array($get_all_consultation)) {
 ...
 <td style="width: 20%; text-align: left;">' . html_entity_decode(htmlentities(patient_name($row["patient_id"]),ENT_QUOTES),ENT_QUOTES) . '</td>
   <td style="text-align: left; width: 20%;">' . html_entity_decode(htmlentities(!empty($row["scheme"]) ? $row["scheme"] : "CASH",ENT_QUOTES),ENT_QUOTES) . '</td>
            <td style="width: 20%; text-align: left;">' . html_entity_decode(htmlentities(drug_name($row["drug_code"]),ENT_QUOTES),ENT_QUOTES) . '</td>
 <td style="width: 20%; text-align: left;">' . html_entity_decode(htmlentities(date("F jS, Y", strtotime($row['date_added'])),ENT_QUOTES),ENT_QUOTES) . '</td>
            </tr>
    </tbody>';
