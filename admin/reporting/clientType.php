...
$clientType = $_SESSION['patientType'];

$date = date('Y-m-d');

if(!empty($clientType)){
    $pageWrite = "$clientType As At ".date("F jS, Y", strtotime($date));   
}
...
$get_all_consultation = clientType($clientType);
...
//        $this->Cell(0, 10, date("d/m/Y h:i:sa") . " " . 'Page ' . $this->getAliasNumPage() . '/' . $this->getAliasNbPages(), 0, false, 'R', 0, '', 0, false, 'T', 'M');
        ...
        $horizontalLine = "<hr />";
        $company = "<strong><em>LAKE CLINIC</em></strong>";
...
<tbody>
      <tr style="background-color: #666; color: #fff;">
       <td style="text-align: left; width: 22%;"><strong>Patient ID</strong></td>
        
         <td style="text-align: left; width: 30%;"><strong>Patient Name</strong></td>
         <td style="text-align: left; width: 25%;"><strong>Insurance</strong></td>
          <td style="text-align: left; width: 23%;"><strong>Membership ID</strong></td>
...
while ($row = mysqli_fetch_array($get_all_consultation)) {
   ...
   // (!empty($row['scheme']) ? $row['scheme'] : 'No Scheme');
   // (!empty($row['membership_id']) ? $row['membership_id'] : 'N/A');
    ...
    <tbody>
        <tr>
          <td style="width: 22%; text-align: left;">' . html_entity_decode(htmlentities($row["patient_id"],ENT_QUOTES),ENT_QUOTES) . '</td>
 <td style="width: 30%; text-align: left;">' . html_entity_decode(htmlentities(patient_name($row["patient_id"]),ENT_QUOTES),ENT_QUOTES) . '</td>
            <td style="width: 25%; text-align: left;">' . html_entity_decode(htmlentities(!empty($row['scheme']) ? $row['scheme'] : 'No Scheme',ENT_QUOTES),ENT_QUOTES) . '</td>
            
            <td style="text-align: left; width: 23%;">' . html_entity_decode(htmlentities((!empty($row['membership_id']) ? $row['membership_id'] : 'N/A'),ENT_QUOTES),ENT_QUOTES) . '</td>
