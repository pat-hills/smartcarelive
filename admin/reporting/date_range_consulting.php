...
if(!empty($startDate) && empty($endDate)){
    $pageWrite = $clientType." "."Consultation As At ".date("F jS, Y", strtotime($startDate));   
}

if(!empty($startDate) && !empty($endDate)){
    $pageWrite = $clientType." "."Consultation From ".date("F jS, Y", strtotime($startDate)). " To ".date("F jS, Y", strtotime($endDate));
}
 


$get_all_consultation = date_range_consultation($startDate,$endDate,$clientType);
...
//$pageTitle = "<strong>" . htmlentities( " " . "Patients Seen Today " . strtoupper("")) . "</strong>";
...
<tbody>
      <tr style="background-color: #666; color: #fff;">
       <td style="text-align: left; width: 12%;"><strong>Patient ID</strong></td>
         ...
         <td style="text-align: left; width: 25%;"><strong>Patient Name</strong></td>
          <td style="text-align: left; width: 20%;"><strong>Scheme</strong></td>
         <td style="text-align: left; width: 28%;"><strong>Seen By</strong></td>
...
while ($row = mysqli_fetch_array($get_all_consultation)) {
 ...
 <td style="width: 25%; text-align: left;">' . html_entity_decode(htmlentities(patient_name($row["patient_id"]),ENT_QUOTES),ENT_QUOTES) . '</td>
            ...
            <td style="width: 28%; text-align: left;">' . html_entity_decode(htmlentities(doctor_name($row["doctor_id"]),ENT_QUOTES),ENT_QUOTES) . '</td>
            
            <td style="text-align: left; width: 15%;">' . html_entity_decode(htmlentities(date("F jS, Y", strtotime($row['date_sent'])),ENT_QUOTES),ENT_QUOTES) . '</td>
</tr>
    </tbody>';
