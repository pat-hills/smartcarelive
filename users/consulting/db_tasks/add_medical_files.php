...
if(isset($doctor_id) && isset($patient_id) && isset($patient_medical_examination) && isset($_FILES['fileToUpload'])){

	if(!empty($doctor_id) && !empty($patient_id) && !empty($patient_medical_examination)){

		if($check_patient_visit_consultation){
			patient_medical_files($patient_id,$doctor_id,$patient_medical_examination,$_FILES['fileToUpload']);
		}else{
