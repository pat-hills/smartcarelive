...
   
        ...
        if( isset($_POST['load_records'])) {
          ...
          $period_amount = number_format(load_sum_expense_report($period,$startDate,$endDate),2);

          $sub_total = "Sub Total";

          $consulation_rows = load_expense_report($period,$startDate,$endDate);
        ...
        if( isset($_POST['load_records'])) {
          ...
          $period_amount = number_format(load_sum_consultation_report($period,$startDate,$endDate),2);
        ...
        if( isset($_POST['load_records'])) {
          ...
          $_SESSION['endDate'] = $endDate;
          $_SESSION['period'] = $period;
          $period_amount = number_format(load_sum_laboratory_report($period,$startDate,$endDate),2);
        ...
        if( isset($_POST['load_records'])) {
          ...
          $_SESSION['endDate'] = $endDate;
          $_SESSION['period'] = $period;
          $period_amount = number_format(load_sum_drugs_report($period,$startDate,$endDate),2);
 ...
 if(isset($_POST['load_records'])){
  ...
  $total = "Total(Profit/Loss)";
     ...
     $expense = load_sum_expense_report($period,$startDate,$endDate);
     $incomes = load_sum_consultation_report($period,$startDate,$endDate) + 
     load_sum_laboratory_report($period,$startDate,$endDate)+ 
      load_sum_drugs_report($period,$startDate,$endDate);

     $profit_loss = $incomes - $expense;
...
<?php
          ...
          if(isset($_POST['load_records'])) { ?>
         <div><a class="btn btn-primary" href="../../users/cashier/reporting/acc_statement" target="_blank">Generate report
