...
if(isset($_POST['FBS'])){
	...
	if($check_blood_fbs_){
     ...
     $is_exist = false;
		...
		$inserted =  insert_blood_fbs($request_code, $patient_id, $lab_staff_id, $BLOOD_FBS, $BLOOD_RBS,$is_exist);		
		if($inserted){
...
$is_exist = false;
		...
		if(isset($_POST['TYROID_FUNCTION_TEST'])){
			...
			if($check_tyroid_func){
				...
				$inserted =  insert_tyroid_func($request_code, $patient_id, $lab_staff_id, $F_T_3, $F_T_4, $T_S_H,$is_exist);
...
if(isset($patient_id) && isset($request_code)){
	...
	send_lab_results_walk_in($patient_id, $request_code, $lab_staff_id, $status, $processed_date);
